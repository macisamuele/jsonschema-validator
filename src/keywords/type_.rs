...
impl Validator for Type {
    ...
    fn validation_errors<T: 'static + JsonType>(&self, path: &str, value: &T) -> ValidationErrorIterator {
        ...
        let is_type_compatible = |primitive_type| {
            if primitive_type == &PrimitiveType::Number {
                // An integer, in the JSON value, should be consiedred a valid number
                value_primitive_type == PrimitiveType::Number || value_primitive_type == PrimitiveType::Integer
            } else {
                primitive_type == &value_primitive_type
            }
        };
        if self.types.iter().any(is_type_compatible) {
...
mod validator_tests {
    ...
    fn number_schema() -> Schema {
        draft4_schema(rust_type!({"type": ["number"]})).expect("Schema is supposed to be valid")
    }
    ...
    #[test_case(
        rust_type!({"type": [1]}),
        "Values have to be of `string` type. `1` has type `integer`."
    )]
    #[test_case(
        rust_type!({"type": 2}),
        "Values have to be of `string` type. `2` has type `integer`."
    )]
    ...
    #[test_case(&number_schema(), &rust_type!(1), &[])]
    #[test_case(&number_schema(), &rust_type!(2.3), &[])]
